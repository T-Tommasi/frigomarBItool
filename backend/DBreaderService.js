class ReaderService {
	static readAnalysisDb() {
		...
		const negativeIncomeProductMap =
			this._identifyNegativeIncomeProducts(productMap);
	...
	static getClientSummaryData(options = {}) {
		...
		try {
			...
			throw new Error(
				"Si è verificato un errore critico durante l'aggregazione dei dati."
			);
		}
	}
	...
	static _readProductIncomeData() {
		...
		if (data.length < 2) {
			Logger.log(
				`ERROR: Insufficient data rows. Expected >= 2, got ${data.length}`
			);
			return null;
		}
	...
	static _processProductClient(product, row, headerMap, clientUUID) {
		...
		const saleIncome = SanitizationServices.sanitizeMoney(
			row[headerMap["Val. Vend."]]
		);
		const saleOriginValue = SanitizationServices.sanitizeMoney(
			row[headerMap["Val. Costo"]]
		);
	...
	static _calculateProductMetrics(productMap) {
		for (const product of Object.values(productMap)) {
			...
			product.percentageIncome =
				product.saleIncome > 0
	...
	static _identifyNegativeIncomeProducts(productMap) {
		...
		for (const product of Object.values(productMap)) {
			if (product.saleIncome <= 0) {
				Logger.log(
					`WARNING: Product ${product.uuid} has negative/zero income: ${product.saleIncome}`
				);
	...
	static _buildClientMap(data, headerMap) {
		...
		for (const row of data) {
			...
			if (!clientMap[clientUUID]) {
				...
				clientMap[clientUUID] = new ClientProductRelationAnalysis(
					clientUUID,
					clientName,
					parseDate
				);
	...
	static _processClientProduct(client, row, headerMap, productUUID) {
		...
		const revenue = SanitizationServices.sanitizeMoney(
			row[headerMap["Val. Vend."]]
		);
		const cost = SanitizationServices.sanitizeMoney(
			row[headerMap["Val. Costo"]]
		);
	...
	static getPartialClientMarginMap(reportConfig) {
		if (!reportConfig || !reportConfig.clientMap) {
			throw new Error("Configurazione del report non valida o mancante.");
		}

		// Get the full map of all clients from the database.
		const { clientMap: allClientsMap } = this.readClientMarginDb();

		if (!allClientsMap) {
			Logger.log(
				"Warning: The complete client margin database is empty or could not be read."
			);
			return {};
		}

		const partialClientMarginMap = {};
		const requestedClientIds = Object.keys(reportConfig.clientMap);

		for (const clientId of requestedClientIds) {
			if (allClientsMap[clientId]) {
				partialClientMarginMap[clientId] = allClientsMap[clientId];
			} else {
				Logger.log(
					`Warning: Client with ID "${clientId}" from report config not found in the main database.`
				);
			}
		}

		return partialClientMarginMap;
	}
}
